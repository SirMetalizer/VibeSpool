...
class StatisticsDialog(tk.Toplevel):
    ...
    def build_ui(self):
        ...
        for item in self.inventory:
            ...
            key = (brand, color_clean)
            if key not in brand_color_stats:
                brand_color_stats[key] = {'count': 0, 'weight': 0.0, 'value': 0.0}
            brand_color_stats[key]['count'] += 1
            brand_color_stats[key]['weight'] += net
            brand_color_stats[key]['value'] += val
        ...
        # Tab 2: Hersteller & Farbe
        tab_brand_color = ttk.Frame(breakdown_notebook)
        breakdown_notebook.add(tab_brand_color, text="Hersteller & Farbe")
        
        tree_brand_color = ttk.Treeview(tab_brand_color, columns=("brand", "color", "count", "weight", "value"), show="headings", height=6)
        for col, head, w in zip(("brand", "color", "count", "weight", "value"), ("Hersteller", "Farbe", "Stk", "Gewicht", "Wert"), (110, 90, 50, 65, 60)): 
            tree_brand_color.heading(col, text=head, command=lambda c=col: self.sort_column(tree_brand_color, c, False))
            tree_brand_color.column(col, width=w, anchor="center" if col in ("count", "weight", "value") else "w")
        tree_brand_color.pack(fill="both", expand=True)
        
        for (brand, color), stats in sorted(brand_color_stats.items(), key=lambda x: (x[0][0].lower(), x[0][1].lower())):
            kg = stats['weight'] / 1000
            tree_brand_color.insert("", "end", values=(brand, color, f"{stats['count']} Stk", f"{kg:.2f} kg", f"{stats['value']:.2f} €"))
